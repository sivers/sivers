...
insert into items (id, metaitem_id, sku, name, price_id, weight) values (5, 1, 'one-WRAP', 'one WRAPPED hardcover', 2, 1);
insert into invoices (id, person_id, created, currency, warehouse, shipcost, total, payment, paydate, payinfo, status, shipdate, shipinfo, shipname, addr1, addr2, city, state, postcode, country, phone, gift_note) values (1, 2, '2025-11-11', 'EUR', 'US', 3.5, 20, 20, '2025-11-11', 'one-blah', 'ship', null, null, 'Boo', 'Boo Str 1', null, 'Booville', null, 'boo123', 'BE', '+33445566', null);
...
insert into lineitems (id, invoice_id, item_id, quantity, price) values (3, 3, 5, 1, 26);
insert into templates (code, template) values ('storm-wrap', '<html>{{{core}}}</html>');
insert into templates (code, template) values ('storm-shipments', '
{{#lines}}
howmany={{howmany}}
item_id={{item_id}}
sku={{sku}}
...
select is(head, e'303\r\nLocation: /login'),
	is(body, null)
from storm.shipments('BadCookieBadCookieBadCookieBadKK');
...
from storm.shipments('StormAdmin1StormAdmin1StormAdmin');
