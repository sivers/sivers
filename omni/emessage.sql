-- CREATE OUTGOING EMAIL MESSAGE TEXT: RFC 5322 - from emails.id
...
-- MIME-Version
-- Content-Type
create function o.emessage(_emailid integer, out msg text) as $$
declare
	r record;
begin
	...
	where e.id = $1
	and (e.outgoing is null or e.outgoing is true);
	if r is null then
		return;
	end if;
	...
	-- empty line between headers and body, and another empty line after
	-- body has to be \r\n everywhere, so erase all \r, then add \r to every \n
	msg = msg || e'\r\n' || replace(replace(r.body, e'\r', ''), e'\n', e'\r\n') || e'\r\n';
end;
$$ language plpgsql;
