...
';
	...
	for r in select uri
		...
		order by uri
		loop
		body = body || '<url><loc>https://sive.rs/' || r.uri || e'</loc><lastmod>2026-05-13</lastmod></url>\n';
	end loop;
	for r in select topic, greatest(max(posted), '2026-05-13'::date) as posted
		from articles
		group by topic
		order by topic
		loop
	...
	for r in select uri, greatest(posted, '2026-05-13'::date) as posted
		from articles
	...
	for r in select code, greatest(read, '2026-05-13'::date) as read
		from ebooks
	...
	for r in select uri, greatest(ymdhm::date, '2026-05-13'::date) as ymd
		from interviews
